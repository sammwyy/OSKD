...
impl Client {
    ...
    pub fn emit(&self, sid: i64, packet: Packet) -> Result<(), Error> {
        ...
        let socket = self.engine.upgrade().unwrap().get_socket(sid);
        if socket.is_some() {
            let socket = socket.unwrap();
            socket.emit(packet.try_into()?).unwrap_or(());
        }
        Ok(())
    }
